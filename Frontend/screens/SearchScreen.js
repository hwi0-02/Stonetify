...
const SearchScreen = () => {
  ...
  const search = async (searchQuery) => {
    ...
    setLoading(true);
    try {
      const trackResultsResponse = await apiService.searchTracks(searchQuery);

      const sections = [];

      if (trackResultsResponse && trackResultsResponse.length > 0) {
        sections.push({
          title: '음원',
          data: trackResultsResponse.map((track) => ({
            ...track,
            type: 'track',
          })),
        });
      ...
      return () => {
        ...
        if (navigation?.setParams) {
          navigation.setParams({
            ...
            targetPlaylistTitle: undefined,
            isCreatingPlaylist: undefined,
            playlistTitle: undefined,
            playlistDescription: undefined,
            newlyCreatedPlaylistId: undefined,
          });
        }
      };
  ...
  const recentSearchSections = useMemo(() => {
    ...
    return tracks.length > 0 ? [{ title: '음원', data: tracks }] : [];
  }, [history]);
