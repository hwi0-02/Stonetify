//FeedScreen  Stonetify/Frontend/screens/FeedScreen.js
...
const FeedScreen = () => {
    ...
    const sortedPosts = useMemo(() => {
        ...
        if (tab === 'popular') {
            // 1. 좋아요가 1개 이상인 게시물만 필터링합니다.
            const popularPosts = postsCopy.filter(post => (post.likesCount || 0) > 0);
            // 2. 필터링된 게시물을 좋아요 순서대로 정렬합니다.
            return popularPosts.sort((a, b) => (b.likesCount || 0) - (a.likesCount || 0));
        }
    ...
    const renderPost = ({ item }) => (
        <PostCard 
            ...
            onSavePress={() => user ? dispatch(toggleSavePost(item.id)) : Alert.alert('로그인이 필요한 기능입니다.')}
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.tabRow}>
                <TouchableOpacity style={[styles.tabItem, tab === 'popular' && styles.tabActive]} onPress={() => setTab('popular')}>
                    <Text style={[styles.tabText, tab === 'popular' && styles.tabTextActive]}>인기글</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.tabItem, tab === 'latest' && styles.tabActive]} onPress={() => setTab('latest')}>
                ...
                ListEmptyComponent={() => (
                    <View style={styles.emptyContainer}>
                        ...
                        <Text style={styles.emptySubText}>
                            {tab === 'popular' ? '나만의 플레이리스트를 공유하여 좋아요를 받아보세요.' : ' 최신글 피드에 좋아요를 눌러보세요.'}
                        </Text>
                    </View>
                )}
            />

            <TouchableOpacity style={styles.fab} onPress={() => navigation.navigate('WriteFeed')}>
