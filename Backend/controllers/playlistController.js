...
const getRandomPlaylists = asyncHandler(async (req, res) => {
    // 1. 모든 공개 플레이리스트를 가져옵니다.
    const allPublicPlaylists = await Playlist.findPublicPlaylists();

    // 2. Fisher-Yates 알고리즘으로 플레이리스트 배열을 무작위로 섞습니다.
    for (let i = allPublicPlaylists.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [allPublicPlaylists[i], allPublicPlaylists[j]] = [allPublicPlaylists[j], allPublicPlaylists[i]];
    }

    // 3. 무작위로 섞인 플레이리스트 중 최대 10개를 선택합니다.
    const randomPlaylists = allPublicPlaylists.slice(0, 10);

    // 4. 선택된 플레이리스트에 커버 이미지를 추가합니다.
    const playlistsWithCovers = await Promise.all(
        randomPlaylists.map(async (playlist) => {
            const songs = await Song.findByPlaylistId(playlist.id);
